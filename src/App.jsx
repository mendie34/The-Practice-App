...
export default function GolfPracticeApp({ onSwitchProfile, profileName, profileId }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const result = await window.storage.get("golf:sessions", false);
      ...
      try {
        const active = await window.storage.get("golf:activeSession", false);
        if (active && active.value) {
          setActiveSaved(JSON.parse(active.value));
        }
      } catch (e) {
        // no in-progress session saved
      } finally {
        setLoaded(true);
      }
    })();
  ...
  // load putting history + any saved in-progress putting session
  useEffect(() => {
    (async () => {
      try {
        const result = await window.storage.get("putting:sessions", false);
        if (result && result.value) {
          setPuttHistory(JSON.parse(result.value));
        }
      } catch (e) {
        // key doesn't exist yet, that's fine
      }
      try {
        const active = await window.storage.get("putting:activeSession", false);
        if (active && active.value) {
          setPuttActiveSaved(JSON.parse(active.value));
        }
      } catch (e) {
        // no in-progress session saved
      }
      try {
        const activeRound = await window.storage.get("putting:activeRound", false);
        if (activeRound && activeRound.value) {
          setOnCourseHoles(JSON.parse(activeRound.value));
        }
      } catch (e) {
        // no in-progress round saved
      } finally {
        setPuttLoaded(true);
      }
    })();
  ...
  // load short game history + any saved in-progress session
  useEffect(() => {
    (async () => {
      try {
        const result = await window.storage.get("shortgame:sessions", false);
        if (result && result.value) {
          setShortHistory(JSON.parse(result.value));
        }
      } catch (e) {
        // key doesn't exist yet, that's fine
      }
      try {
        const active = await window.storage.get("shortgame:activeSession", false);
        if (active && active.value) {
          setShortActiveSaved(JSON.parse(active.value));
        }
      } catch (e) {
        // no in-progress session saved
      } finally {
  ...
  // load persisted settings (baseline handicap + units + range tracking mode)
  useEffect(() => {
    (async () => {
      try {
        const result = await window.storage.get("settings:preferences", false);
        if (result && result.value) {
        ...
        // no saved preferences yet, defaults apply
      } finally {
  ...
  async function handleExportData() {
    if (!profileId) return;
    await exportProfileData(profileId, profileName || "profile");
  }
    ...
    >
      ...
      <div style={{ width: "100%", maxWidth: 420 }}>
        <Header
          screen={screen}
          onAnalysis={() => {
            if (screen === "putting" || screen === "puttingPractice" || screen === "puttingSummary") {
              setAnalysisSection("putting");
            } else if (screen === "shortgame" || screen === "shortGamePractice" || screen === "shortGameSummary") {
              setAnalysisSection("shortgame");
            } else if (screen === "setup" || screen === "practice" || screen === "summary") {
...
function Header({ screen, onAnalysis, onSettings, onHome }) {
  return (
    <div
      className="no-print"
  ...
  return (
    <div>
      ...
      <Card style={{ marginTop: 12 }}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
          <button onClick={onExportData} style={settingsGhostButtonStyle}>
            EXPORT DATA
          </button>
          <label style={{ ...settingsGhostButtonStyle, textAlign: "center", display: "block" }}>
            IMPORT DATA
            <input
              type="file"
...
const settingsGhostButtonStyle = {
  flex: 1,
  padding: "10px 0",
  borderRadius: 10,
  border: `1px solid ${COLORS.creamDim}33`,
...
const TILE_META = [
  { key: "range", label: "THE RANGE", subtitle: "Distance control practice", screen: "setup", available: true, Illustration: RangeIllustration },
  {
    key: "shortgame",
    label: "SHORT GAME",
...
function PuttingSetupScreen({
  ...
  const invalidRange = puttMinFt >= puttMaxFt || puttMinFt < 0;
