...
// ===== Demo-only profile layer =====
// The standalone deployed version of this app uses IndexedDB for this (see storage.js in the
// project you downloaded). Inside this Claude artifact preview, only window.storage is
// available, so profiles are demonstrated here by namespacing keys within window.storage
// instead — same UX, different plumbing. Every existing window.storage.get/set/delete/list call
// throughout the app below is untouched; this proxy intercepts them transparently.
//
// IMPORTANT: the real window.storage is captured lazily, on first actual use, rather than at
// module load time. Capturing it immediately when this file first parses is a race condition —
// if Claude's storage API isn't fully wired into the page yet at that exact instant, the capture
// silently becomes undefined and every storage call fails forever afterward with no visible
// error. Deferring the capture until the first real call (always well after mount, triggered by
// user interaction) makes that failure mode effectively impossible.
let REAL_STORAGE = null;
let proxyInstalled = false;
let activeProfileId = null;

// Fallback used only if window.storage genuinely isn't available at all (not just delayed) —
// e.g. viewing this without being signed into a paid Claude plan, which the real storage API
// requires. Without this, the app would hang forever with no visible error. With it, everything
// still works for the current session; it just won't survive a page refresh. This has no bearing
// on the real deployed app, which uses IndexedDB and doesn't depend on Claude's storage at all.
function createMemoryStorageFallback() {
  ...
  return {
    ...
    async list(prefix) {
      const p = prefix || "";
      const keys = [...memory.keys()].filter((k) => k.startsWith(p)).map((k) => k.slice(p.length));
      return { keys };
    },
  };
}

function ensureStorageProxy() {
  if (proxyInstalled) return;
  REAL_STORAGE = window.storage || createMemoryStorageFallback();
  window.storage = {
    async get(key, shared) {
      return REAL_STORAGE.get(`profile:${activeProfileId}:${key}`, shared);
    },
    async set(key, value, shared) {
      return REAL_STORAGE.set(`profile:${activeProfileId}:${key}`, value, shared);
    },
    async delete(key, shared) {
      return REAL_STORAGE.delete(`profile:${activeProfileId}:${key}`, shared);
    },
    async list(prefix, shared) {
      return REAL_STORAGE.list(`profile:${activeProfileId}:${prefix || ""}`, shared);
    },
  };
  proxyInstalled = true;
}

function setActiveProfileIdForStorage(id) {
  ensureStorageProxy();
  activeProfileId = id;
}

async function listProfilesDemo() {
  ensureStorageProxy();
  try {
    const result = await REAL_STORAGE.get("app:profiles", false);
...
async function createProfileDemo(name, handicap) {
  ensureStorageProxy();
  const profiles = await listProfilesDemo();
  const profile = {
  ...
  await REAL_STORAGE.set("app:profiles", JSON.stringify(profiles), false);
  return profile;
}

async function deleteProfileDemo(id) {
  ensureStorageProxy();
  const profiles = await listProfilesDemo();
  const next = profiles.filter((p) => p.id !== id);
  await REAL_STORAGE.set("app:profiles", JSON.stringify(next), false);
  try {
    const listed = await REAL_STORAGE.list(`profile:${id}:`, false);
    if (listed && listed.keys) {
      await Promise.all(listed.keys.map((k) => REAL_STORAGE.delete(`profile:${id}:${k}`, false)));
    }
  } catch (e) {
    // non-fatal — orphaned keys just sit unused
  }
}

async function exportProfileDataDemo(profileId, profileName) {
  ensureStorageProxy();
  const listed = await REAL_STORAGE.list(`profile:${profileId}:`, false);
  const entries = {};
  if (listed && listed.keys) {
    for (const k of listed.keys) {
      const result = await REAL_STORAGE.get(`profile:${profileId}:${k}`, false);
      if (result) entries[k] = result.value;
    }
  }
  const payload = { app: "the-practice-app", exportedAt: new Date().toISOString(), profileName, data: entries };
...
async function importProfileDataDemo(profileId, file) {
  ensureStorageProxy();
  const text = await file.text();
  const payload = JSON.parse(text);
  if (!payload || !payload.data) throw new Error("That doesn't look like a Practice App export file.");
  const entries = Object.entries(payload.data);
  await Promise.all(entries.map(([key, value]) => REAL_STORAGE.set(`profile:${profileId}:${key}`, value, false)));
  return entries.length;
}

// Consolidated bulk load: fetches every one of this profile's known data keys in parallel rather
// than each screen loading its own key one at a time in sequence on mount. Cuts app-open from
// ~9 sequential network round-trips down to 1 batch of parallel ones.
const APP_DATA_KEYS = [
  "golf:sessions",
  "golf:activeSession",
  "putting:sessions",
...
async function loadAllAppDataDemo() {
  ensureStorageProxy();
...
function GolfPracticeApp({ onSwitchProfile, profileName, profileId, profileHandicap }) {
  ...
  useEffect(() => {
    (async () => {
      const data = await loadAllAppDataDemo();
      ...
      <div style={{ width: "100%", maxWidth: 420 }}>
        ...
        {screen === "settings" && (
          <SettingsScreen
            ...
            onExportData={() => exportProfileDataDemo(profileId, profileName)}
            onImportData={async (file) => {
              try {
                const count = await importProfileDataDemo(profileId, file);
...
function ProfileSetupWizard({ onComplete }) {
...
function ProfileGateDemo() {
  ...
  useEffect(() => {
    (async () => {
      const list = await listProfilesDemo();
      setProfiles(list);
      setLoaded(true);
    })();
  }, []);
  ...
  function selectProfile(id) {
    setActiveProfileIdForStorage(id);
    setActiveId(id);
  }
  ...
  async function handleWizardComplete({ name, handicap, rangeTrackingMode, baselineHandicap }) {
    let profile;
    try {
      profile = await createProfileDemo(name, handicap);
    } catch (e) {
      // Even the fallback storage shouldn't throw, but if something truly unexpected happens,
      // fall back to an in-memory-only profile object so the wizard still completes rather than
      // hanging with no feedback.
      profile = { id: Math.random().toString(36).slice(2, 10) + Date.now().toString(36), name: name.trim(), handicap: handicap || null, createdAt: new Date().toISOString() };
    }
    setProfiles((prev) => [...prev, profile]);
    // Activate this profile's storage scope so the settings write below lands in the right place,
    // and so GolfPracticeApp picks these up as already-answered when it mounts and loads settings.
    setActiveProfileIdForStorage(profile.id);
    try {
      await window.storage.set(
        "settings:preferences",
  ...
  async function handleDelete(id) {
    await deleteProfileDemo(id);
    setProfiles((prev) => prev.filter((p) => p.id !== id));
    setConfirmDeleteId(null);
  }

  function handleSwitchProfile() {
    setActiveProfileIdForStorage(null);
    setActiveId(null);
  }
  ...
  if (activeId) {
    ...
    return (
      <GolfPracticeApp
        ...
        profileId={activeId}
        profileHandicap={profile?.handicap ?? null}
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div style={{ width: "100%", maxWidth: 420 }}>
        ...
        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, color: COLORS.creamDim, marginBottom: 6 }}>
          Who's playing? Data stays separate per profile.
        </div>
        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: COLORS.sand, marginBottom: 20, lineHeight: 1.5 }}>
          Demo note: this preview uses Claude's storage, namespaced per profile. The version you
          downloaded uses IndexedDB instead — same behavior, real standalone storage.
        </div>
